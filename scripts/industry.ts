...
function buildIndustryPrompt(url: string, name: string): string {
  return `あなたはビジネスアナリストです。このWebサイトのスクリーンショットを見て、業種と業態を分類してください。
...
## 分類ルール
- industry_category: 以下の候補から最も適切な1つを選ぶ（必須）
- industry_tags: このサイトが「個人・中小規模」か「大手チェーン・企業」かを区別するため、
  以下の業態サブタグから当てはまるものを1〜2個選ぶ
  ※ サブタグは上記の業種カテゴリに対応するものが定義されている。業種と一致するタグのみ選ぶ
  ※ 候補に該当するタグがない場合は空配列 [] を返す

## 業種カテゴリ（候補・1つ選択）
${INDUSTRY_CATEGORIES.join("、")}

## 業態サブタグ（候補・1〜2個選択。業種が合致するもののみ）
${INDUSTRY_TAGS.join("、")}
