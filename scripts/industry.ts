...
// ============================================================
// 業態サブタグ定義（DATABASE_DESIGN.md ① 1-b と同期）
// 個人・中小規模 vs 大手を明確に区別してSkovaターゲット精度を向上させる
// ============================================================
const INDUSTRY_TAGS = [
  // 飲食・フード
  "個人カフェ・コーヒー専門店",
...
function buildIndustryPrompt(url: string, name: string): string {
  ...
  try {
    ...
    domainHint = parsed.hostname.replace("www.", "");
  } catch {
    domainHint = url;
  }

  return `あなたはビジネスアナリストです。このWebサイトのスクリーンショットとサイト情報を見て、業種と業態を分類してください。

## サイト情報
- URL: ${url}（ドメイン: ${domainHint}）
- サイト名: ${name ?? "不明"}

## 分類ルール

### industry_category（必須・1つ選択）
以下の候補から最も適切な1つを選びます。

${INDUSTRY_CATEGORIES.join("、")}

### industry_tags（1〜2個選択・業種に対応するもののみ）
このサイトが「個人・小規模事業者」か「大手チェーン・上場企業」かを区別するためのタグです。
**判定基準**:
- 個人・中小規模の特徴: 店舗数が少ない（1〜数店舗）・代表者の顔写真・手作り感のあるデザイン・地域名が強調される・予約は電話/LINE中心
- 大手チェーンの特徴: 全国展開・フランチャイズ・上場企業・100店舗以上・コーポレートサイト的なデザイン

以下から当てはまるものを1〜2個選びます（業種カテゴリと一致するタグのみ選ぶこと）:
${INDUSTRY_TAGS.join("、")}

**候補に合致するタグがない場合は空配列 [] を返してください。**

## 出力形式（このJSONのみ。説明文・コメント不要）
{
